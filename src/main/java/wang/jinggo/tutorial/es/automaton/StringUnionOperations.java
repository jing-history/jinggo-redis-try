package wang.jinggo.tutorial.es.automaton;

import wang.jinggo.tutorial.es.automaton.Transition;
...
final public class StringUnionOperations {
	 ...
	 */
	private static wang.jinggo.tutorial.es.automaton.State convert(State s,
			IdentityHashMap<State, wang.jinggo.tutorial.es.automaton.State> visited) {
		wang.jinggo.tutorial.es.automaton.State converted = visited.get(s);
		if (converted != null)
			return converted;

		converted = new wang.jinggo.tutorial.es.automaton.State();
	 ...
	 */
	public static wang.jinggo.tutorial.es.automaton.State build(CharSequence[] input) {
		final StringUnionOperations builder = new StringUnionOperations();

		for (CharSequence chs : input)
			builder.add(chs);

		return convert(builder.complete(), new IdentityHashMap<State, wang.jinggo.tutorial.es.automaton.State>());
	}
