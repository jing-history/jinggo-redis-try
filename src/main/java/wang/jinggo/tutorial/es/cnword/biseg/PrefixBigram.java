 ...
 **/
public class PrefixBigram {
    private static PrefixBigram dic = null;

    /**
     *
     * @return the singleton of basic dictionary
     */
    public static PrefixBigram getInstance(String dicDir) {
        if (dic == null) {
            try {
                dic = new PrefixBigram(dicDir);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public void loadBigramDictionay(String path) throws Exception {
        ...
        while ((line = read.readLine()) != null) {
            ...
            if ((prefixNode == null) || (prefixNode.data == null)) {// 在一元词典找后缀节点找不到去掉
				/*
            ...
            id = suffixNode.data.biEntry.id; /* 记录前缀单词的id */
            frq = Integer.parseInt(st.nextToken()); /* 记录单词的频率 */

            prefixNode.data.biEntry.put(id, frq);
			/*
    ...
    public static final String binDic = "PrefixBigram.bin";

    public PrefixBigram(String dicDir) throws Exception {
    ...
    public int getBigramFreq(String prev, String next) {
        ...
        int frq = prevNode.data.biEntry.get(nextNode.data.biEntry.id);
