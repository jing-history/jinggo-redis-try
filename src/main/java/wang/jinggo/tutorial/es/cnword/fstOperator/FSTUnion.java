package wang.jinggo.tutorial.es.cnword.fstOperator;
...
import wang.jinggo.tutorial.es.cnword.fst.CharSpan;
import wang.jinggo.tutorial.es.cnword.fst.FST;
import wang.jinggo.tutorial.es.cnword.fst.State;
...
public class FSTUnion {
	...
	// 对两个输入集合求并集
	private static Set<CharSpan> union(Set<wang.jinggo.tutorial.es.cnword.fst.Transition> setA,
			Set<wang.jinggo.tutorial.es.cnword.fst.Transition> setB) {
		Set<CharSpan> tmp = new HashSet<CharSpan>();
		//if (setA != null) {
			for (wang.jinggo.tutorial.es.cnword.fst.Transition x : setA) {
				tmp.add(new CharSpan(x.min,x.max));
			}
		//}
		//if (setB != null) {
			for (wang.jinggo.tutorial.es.cnword.fst.Transition x : setB) {
				tmp.add(new CharSpan(x.min,x.max));
			}
		//}
		return tmp;
	...
	public static Set<CharSpan> getCharSpans(Set<wang.jinggo.tutorial.es.cnword.fst.Transition> transitions){
